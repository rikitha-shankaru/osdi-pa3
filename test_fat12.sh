...
echo "\nTest 4: Verify file contents"
if cmp test1.txt output1.txt > /dev/null 2>&1; then
    echo "TEST1.TXT: Files match [OK]"
else
    echo "TEST1.TXT: Files differ [FAIL]"
    exit 1
fi

if cmp test2.txt output2.txt > /dev/null 2>&1; then
    echo "TEST2.TXT: Files match [OK]"
else
    echo "TEST2.TXT: Files differ [FAIL]"
    exit 1
fi
